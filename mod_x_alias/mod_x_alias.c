// *
// * XAlias module :: mod_alias: Just changed 'APR_HOOK_MIDDLE' to 'APR_HOOK_FIRST' !!
...
module AP_MODULE_DECLARE_DATA x_alias_module; //***
...
static const char *add_alias_internal(cmd_parms *cmd, void *dummy,  const char *fake, const char *real, int use_regex){
	...
	alias_server_conf *conf = ap_get_module_config(s->module_config, &x_alias_module);
...
static const char *add_alias(cmd_parms *cmd, void *dummy, const char *fake,
	...
	if (real) {
		...
		dirconf->alias =
				ap_expr_parse_cmd(cmd, fake, AP_EXPR_FLAG_STRING_RESULT, &err, NULL);
		if (err) {
			return apr_pstrcat(cmd->temp_pool, "Cannot parse alias expression '", fake, "': ", err, NULL);
		}
...
static const char *add_redirect_internal(cmd_parms *cmd, alias_dir_conf *dirconf, const char *arg1, const char *arg2, const char *arg3, int use_regex){
	alias_entry *new;
	server_rec *s = cmd->server;
	alias_server_conf *serverconf = ap_get_module_config(s->module_config, &x_alias_module);
	...
	if (!cmd->path) {
		...
		dirconf->redirect =
				ap_expr_parse_cmd(cmd, url, AP_EXPR_FLAG_STRING_RESULT, &expr_err, NULL);
		if (expr_err) {
			return apr_pstrcat(cmd->temp_pool, "Cannot parse redirect expression '", url, "': ", expr_err, NULL);
		}
...
static const command_rec alias_cmds[] ={
	AP_INIT_TAKE12("XAlias", add_alias, NULL, RSRC_CONF | ACCESS_CONF,
				 "a fakename and a realname, or a realname in a Location"),
	AP_INIT_TAKE12("XScriptAlias", add_alias, "cgi-script", RSRC_CONF | ACCESS_CONF,
				 "a fakename and a realname, or a realname in a Location"),
	AP_INIT_TAKE123("XRedirect", add_redirect, (void *) HTTP_MOVED_TEMPORARILY,
				 OR_FILEINFO,
				 "an optional status, then document to be redirected and "
				 "destination URL"),
	AP_INIT_TAKE2("XAliasMatch", add_alias_regex, NULL, RSRC_CONF,
				 "a regular expression and a filename"),
	AP_INIT_TAKE2("XScriptAliasMatch", add_alias_regex, "cgi-script", RSRC_CONF,
				 "a regular expression and a filename"),
	AP_INIT_TAKE23("XRedirectMatch", add_redirect_regex,
				 (void *) HTTP_MOVED_TEMPORARILY, OR_FILEINFO,
				 "an optional status, then a regular expression and "
				 "destination URL"),
	AP_INIT_TAKE2("XRedirectTemp", add_redirect2,
				 (void *) HTTP_MOVED_TEMPORARILY, OR_FILEINFO,
				 "a document to be redirected, then the destination URL"),
	AP_INIT_TAKE2("XRedirectPermanent", add_redirect2,
...
static char *try_alias(request_rec *r){
	alias_dir_conf *dirconf =
			(alias_dir_conf *) ap_get_module_config(r->per_dir_config, &x_alias_module);
	...
	if (dirconf->alias) {
		...
		if (err) {
			ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, APLOGNO(02825) "Can't evaluate alias expression: %s", err);
...
static char *try_redirect(request_rec *r, int *status){
	...
	if (dirconf->redirect_set) {
		...
		if (dirconf->redirect) {
			...
			found = apr_pstrdup(r->pool, ap_expr_str_exec(r, dirconf->redirect, &err));
			if (err) {
...
static int translate_alias_redir(request_rec *r){
	...
	alias_server_conf *serverconf = ap_get_module_config(sconf, &x_alias_module);
	char *ret;
	int status;
...
static int fixup_redir(request_rec *r){
	...
	alias_dir_conf *dirconf =
	(alias_dir_conf *) ap_get_module_config(dconf, &x_alias_module);
	char *ret;
	int status;
...
static void register_hooks(apr_pool_t *p){
	static const char * const aszSucc[]={ "mod_userdir.c", "mod_vhost_alias.c",NULL };

	ap_hook_translate_name(translate_alias_redir,NULL,aszSucc,APR_HOOK_FIRST);
	ap_hook_fixups(fixup_redir,NULL,NULL,APR_HOOK_FIRST);
}

AP_DECLARE_MODULE(x_alias) ={
