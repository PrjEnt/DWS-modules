...
inline int PreapreProcess(request_rec *r, STARTUPINFO* si, HANDLE *HStdOut, HANDLE *HStdIn, HANDLE *HStdErr, char **ScriptCommand){
	...
	if(! CreatePipe(&HStdErr[Pipe_Self], &HStdErr[Pipe_Child], &sa, 0)){ //Self <--> Child
		Log(r, APLOG_ERR, "mod_x_cgi", "Failed to create STDERR pipe.");
		return 500;
	}
	...
	si->hStdError = HStdErr[Pipe_Child];
