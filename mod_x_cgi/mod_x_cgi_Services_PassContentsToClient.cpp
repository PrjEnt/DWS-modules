...
inline int PassContentsToClient(request_rec *r, void* Data, int DataSize, HANDLE* HStdOut, HANDLE* HStdErr, OutputHeaderResults *OHR){
	//Structs
	...
	//Send already read contents (read in header parser)
	...
	while(1){
		...
		b = apr_bucket_heap_create( (const char*)Data, OHR->ReadSize, 0, ba);
		APR_BRIGADE_INSERT_TAIL(bb, b);
	}
/*
	if( ReadFile(HStdErr[0], Data, DataSize-1, ( (DWORD*)(&(OHR->ReadSize)) ), 0) ){
		b = apr_bucket_heap_create( (const char*)Data, OHR->ReadSize, 0, ba);
		APR_BRIGADE_INSERT_TAIL(bb, b);
	}
	*/
